...
REINHARD_ROOT = "/work/h2020deciderficarra_shared/gcorso/wdkarim/stain_norm/norm_patches/reinhard/"

OUT_RAW = "/homes/wdkarim/color_statistics/color_stats_raw.csv"
OUT_REINHARD = "/homes/wdkarim/color_statistics/color_stats_reinhard.csv"
...
def run(root, out_csv, stain_condition):
    ...
    print(f"Saved {out_path} with {len(out)} slides")


def main():
    Path(OUT_RAW).parent.mkdir(parents=True, exist_ok=True)
    Path(OUT_REINHARD).parent.mkdir(parents=True, exist_ok=True)

    run(RAW_ROOT, OUT_RAW, "raw")
    run(REINHARD_ROOT, OUT_REINHARD, "reinhard")
