...
INPUT_CSV = "/homes/smdaniyal/stain_norm/feature_statistics/slide_embeddings_raw_macenko.csv"
OUT_DIR = Path("/homes/smdaniyal/stain_norm/feature_statistics")
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_TSNE_CSV = OUT_DIR / "tsne_by_tss_raw_macenko.csv"
OUT_RAW_PNG = OUT_DIR / "tsne_raw_tss.png"
OUT_MAC_PNG = OUT_DIR / "tsne_macenko_tss.png"
OUT_COMBINED_PNG = OUT_DIR / "tsne_raw_vs_macenko_tss.png"
...
def main():
    ...
    mac_df = tsne_df[tsne_df["stain_condition"] == "macenko"].copy()
    ...
    fig, ax = plt.subplots(figsize=(10, 8))
    make_scatter(ax, mac_df, "UNI t-SNE by TSS (Macenko)")
    ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8, frameon=False)
    fig.tight_layout()
    fig.savefig(OUT_MAC_PNG, dpi=300, bbox_inches="tight")
    plt.close(fig)

    fig, axes = plt.subplots(1, 2, figsize=(18, 8), sharex=False, sharey=False)
    make_scatter(axes[0], raw_df, "Raw")
    make_scatter(axes[1], mac_df, "Macenko")
    ...
    fig.suptitle("UNI t-SNE by TSS (Raw vs Macenko)", fontsize=14)
