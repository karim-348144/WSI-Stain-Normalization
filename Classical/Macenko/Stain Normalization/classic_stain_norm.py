        ...
        for p in tqdm(files, desc=f"{args.method}:{slide_id}", leave=False, disable=args.quiet):
            ...
            try:
                ...
                if out is None or out.size == 0:
                    raise RuntimeError("empty normalized image")

                if not np.isfinite(out).all():
                    raise RuntimeError("non-finite values in normalized image")
                ...
                n_fail += 1
                if not args.quiet:
                    print(f"WARN patch_failed slide={slide_id} patch={p} error={repr(e)}")
                continue

        if not args.quiet:
            print(
                f"SLIDE_DONE slide={slide_id} ok={n_ok} "
                f"skip_existing={n_skip_existing} failed={n_fail}"
            )
