...
STAINGAN_FEAT_ROOT = "/work/h2020deciderficarra_shared/gcorso/wdkarim/stain_norm/StainGAN/features"

OUT_DIR = Path("/work/h2020deciderficarra_shared/gcorso/wdkarim/stain_norm/StainGAN/feature_statistics")
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_SLIDE_EMB = OUT_DIR / "slide_embeddings_raw_staingan.csv"
OUT_BATCH_METRICS = OUT_DIR / "feature_batch_effect_metrics_raw_vs_staingan.csv"
OUT_TSS_COUNTS = OUT_DIR / "feature_batch_effect_tss_counts_raw_vs_staingan.csv"
OUT_RAW_SLIDE = OUT_DIR / "feature_batch_effect_raw.csv"
OUT_STAINGAN_SLIDE = OUT_DIR / "feature_batch_effect_staingan.csv"
...
def main():
    ...
    staingan_df = build_slide_embedding_df(STAINGAN_FEAT_ROOT, "staingan", manifest_df)

    raw_df.to_csv(OUT_RAW_SLIDE, index=False)
    staingan_df.to_csv(OUT_STAINGAN_SLIDE, index=False)
    ...
    staingan_metrics, staingan_used = compute_metrics(staingan_df, "staingan")

    metrics_df = pd.DataFrame([raw_metrics, staingan_metrics])

    if {"raw", "staingan"}.issubset(set(metrics_df["stain_condition"])):
        mraw = metrics_df[metrics_df["stain_condition"] == "raw"].iloc[0]
        mstaingan = metrics_df[metrics_df["stain_condition"] == "staingan"].iloc[0]

        delta_row = {
            "stain_condition": "delta_staingan_minus_raw",
            "n_slides_total": np.nan,
            "n_slides_used": np.nan,
            "n_tss_total": np.nan,
            "n_tss_used": np.nan,
            "embedding_dim": np.nan,
            "silhouette_tss": mstaingan["silhouette_tss"] - mraw["silhouette_tss"],
            "knn_tss_acc_mean": mstaingan["knn_tss_acc_mean"] - mraw["knn_tss_acc_mean"],
            "knn_tss_acc_std": mstaingan["knn_tss_acc_std"] - mraw["knn_tss_acc_std"],
        ...
        pct_row = {
            "stain_condition": "percent_change_staingan_vs_raw",
            "n_slides_total": np.nan,
            "n_slides_used": np.nan,
            "n_tss_total": np.nan,
            "n_tss_used": np.nan,
            "embedding_dim": np.nan,
            "silhouette_tss": (
                100.0 * (mstaingan["silhouette_tss"] - mraw["silhouette_tss"]) / mraw["silhouette_tss"]
                if mraw["silhouette_tss"] != 0 else np.nan
            ),
            "knn_tss_acc_mean": (
                100.0 * (mstaingan["knn_tss_acc_mean"] - mraw["knn_tss_acc_mean"]) / mraw["knn_tss_acc_mean"]
    ...
    tss_counts = pd.concat([
        ...
        staingan_used.groupby("tss").size().rename("n_slides_staingan")
    ], axis=1).reset_index()
    tss_counts.to_csv(OUT_TSS_COUNTS, index=False)

    print(f"Saved: {OUT_RAW_SLIDE}")
    print(f"Saved: {OUT_STAINGAN_SLIDE}")
