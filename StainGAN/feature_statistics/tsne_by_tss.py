...
INPUT_CSV = "/work/h2020deciderficarra_shared/gcorso/wdkarim/stain_norm/StainGAN/feature_statistics/slide_embeddings_raw_staingan.csv"
OUT_DIR = Path("/work/h2020deciderficarra_shared/gcorso/wdkarim/stain_norm/StainGAN/feature_statistics")
OUT_DIR.mkdir(parents=True, exist_ok=True)

OUT_TSNE_CSV = OUT_DIR / "tsne_by_tss_raw_staingan.csv"
OUT_RAW_PNG = OUT_DIR / "tsne_raw_tss.png"
OUT_STAINGAN_PNG = OUT_DIR / "tsne_staingan_tss.png"
OUT_COMBINED_PNG = OUT_DIR / "tsne_raw_vs_staingan_tss.png"
...
def main():
    ...
    staingan_df = tsne_df[tsne_df["stain_condition"] == "staingan"].copy()
    ...
    fig, ax = plt.subplots(figsize=(10, 8))
    make_scatter(ax, staingan_df, "UNI t-SNE by TSS (StainGAN)")
    ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8, frameon=False)
    fig.tight_layout()
    fig.savefig(OUT_STAINGAN_PNG, dpi=300, bbox_inches="tight")
    plt.close(fig)

    fig, axes = plt.subplots(1, 2, figsize=(18, 8), sharex=False, sharey=False)
    make_scatter(axes[0], raw_df, "Raw")
    make_scatter(axes[1], staingan_df, "StainGAN")
    ...
    fig.suptitle("UNI t-SNE by TSS (Raw vs StainGAN)", fontsize=14)
    ...
    print(f"Saved: {OUT_STAINGAN_PNG}")
